...
//            	        			measuredHeight, mSelectedPaint);
		...
		System.out.println("selectAreas====="+selectAreas.size());

		for (int i = 0; i <selectAreas.size() ; i++) {
			float[] floats = selectAreas.get(cutPoint.get(i));
if(null!=floats){
	System.out.println("index====="+floats[0]+"======="+floats[1]);
}
		}
     ...
     * @param position
     */
    public void setCutPostion(int position,int pro){
...
//    	mHandler.sendEmptyMessageDelayed(1, 100);
    ...
    public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			touchX1=event.getX();
			System.out.println( "touchX1====="+touchX1);
			break;
