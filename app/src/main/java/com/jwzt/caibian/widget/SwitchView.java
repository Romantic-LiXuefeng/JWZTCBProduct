...
public class SwitchView extends View {
	...
	private OnChangerListener switchChangeListener; // 监控开关状态
	...
	protected void onDraw(Canvas canvas) {
		//获取自定义颜色
		int my_color = getResources().getColor(R.color.a5);
//		int my_color=Color.parseColor("#b72b2e");
		super.onDraw(canvas);
		paint.setAntiAlias(true);
		final boolean isOn = (state == STATE_SWITCH_ON || state == STATE_SWITCH_ON2);
		// draw background
		paint.setStyle(Style.FILL);
		paint.setColor(isOn ? my_color : 0xffe3e3e3);
		...
		// this color is not encouraged to change,it is just the white color;
		...
		// this should be the key procedure to change the color,it is the ring color,there is another place to replace the color ; 

		paint.setColor(isOn ? my_color : 0xffbfbfbf);
		canvas.drawPath(bPath, paint);
	...
	private void refreshState(int newState) {
		if (!isOpened && newState == STATE_SWITCH_ON) {
			...
			if(switchChangeListener!=null){
				switchChangeListener.onSwitchChanged(this,isOpened);
			}
	...
	public interface OnChangerListener{
		void onSwitchChanged(SwitchView switchView, boolean open);
	}
	
	public void setOnSwitchChangeListener(OnChangerListener onChangerListener) {
		this.switchChangeListener = onChangerListener;
	}
...
//			System.out.println("flagflagflag"+isOpened);
		}
...
//			System.out.println("flagflagflag2"+isOpened);
		}
	};
