...
public class MVideoProgressView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
	/**¼��ʱ��(��λΪ��)*/
	...
	public MVideoProgressView(Context context) {
		super(context);
		init(context);
	}

	public MVideoProgressView(Context paramContext, AttributeSet paramAttributeSet) {
		super(paramContext, paramAttributeSet);
		init(paramContext);

	}

	public MVideoProgressView(Context paramContext, AttributeSet paramAttributeSet, int paramInt) {
...
//		perWidth = screenWidth / Conf.MAX_RECORD_TIME;
		perWidth = screenWidth / (maxRecordTime*1000f);
		...
		minTimePaint.setColor(Color.parseColor("#00ff0000"));//͸��
	...
	private void myDraw() {
		...
		if (timeList.isEmpty() || (!timeList.isEmpty() && timeList.getLast() <= Configs.MIN_RECORD_TIME)) {
			float left = perWidth * Configs.MIN_RECORD_TIME;
			if (canvas != null) {
