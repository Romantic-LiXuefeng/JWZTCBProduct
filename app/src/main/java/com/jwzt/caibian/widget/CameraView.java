...
import com.jwzt.caibian.interfaces.CameraOperation;
...
public class CameraView extends SurfaceView implements CameraOperation{
	...
	private void setCameraParameters(){
		...
		int position = mSharePreferenceUtils.getInt(mContext, GlobalContants.PICTUREQUALITYPOSION, 0);

		if (sizeList.size()>0) {
			
			//获取在图片质量中设置的参数
			if(position==0){
				Size cameraSize=sizeList.get(sizeList.size()-2);;

				parameters.setPictureSize(cameraSize.width, cameraSize.height);
			}else if(position==1){
				Size cameraSize=sizeList.get(sizeList.size()-3);
				parameters.setPictureSize(cameraSize.width, cameraSize.height);
			}else if(position==2){
				Size cameraSize=sizeList.get(1);
				parameters.setPictureSize(cameraSize.width, cameraSize.height);
			}else if(position==3){
				Size cameraSize=sizeList.get(1);
				parameters.setPictureSize(cameraSize.width, cameraSize.height);
