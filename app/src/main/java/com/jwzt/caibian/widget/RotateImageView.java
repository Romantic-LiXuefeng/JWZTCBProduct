...
import com.jwzt.caibian.util.PaintUtil;
...
public class RotateImageView extends View {
	...
	public void draw(Canvas canvas) {
		...
		// 绘制图形
		canvas.save();
		canvas.scale(scale, scale, centerX,
				centerY);
		canvas.drawRect(wrapRect, bottomPaint);
		canvas.rotate(rotateAngle, centerX,
				centerY);
...
//				canvas.getHeight() >> 1);
	...
	private void calculateWrapBox() {
		...
		centerX = (int) ((dstRect.right-dstRect.left)/2);
		centerY = (int) (((dstRect.bottom-dstRect.top)/2)+dstRect.top);
