 ...
 *
 */
public class EditVedioActivity2 extends Activity implements OnClickListener, ScrollViewListener {
	...
	private Handler mHandler=new Handler(){
		...
		public void dispatchMessage(Message msg) {
			...
			case 0://拖动更新
				...
				System.out.println((currentPosition*1.0f/mDuration)+"===============");
	...
	private void initView() {
		...
		mVedioBar = (VideoEditProgressBar)this.findViewById(R.id.vedio_progress);
...
//		mVideoView.seekTo(1);
	...
	public void onClick(View v) {
		...
		case R.id.iv_merge://合并按钮
			if(isEdit){//如果被编辑了
				...
				Toast.makeText(EditVedioActivity2.this, "您还未对视频进行编辑！", 0).show();
			}
			break;
    ...
    private void setCurrentPostion(){
    	mImageLists.setCutPostion(mCurrentPosition,0);
    }
...
//			mVideoView.seekTo(mDragPosition);
	...
	class ExtractTask extends AsyncTask<Void, Void, Bitmap>{
		...
		protected void onPreExecute() {
			...
			if(progressDialog1==null){
				progressDialog1 = ProgressDialog.show(EditVedioActivity2.this,
						"加载中...", "请稍等...", true);
			}else{
    ...
    private void resetFlag(List<long[]> cutPostion_use1) {
    	 List<Integer> flags = info.getFlags();
    	 List<Integer> flags_temp=new ArrayList<Integer>();
    	
    	if(flags!=null&&flags.size()>0){
    		for(int i=0;i<flags.size();i++){
    			int temp=flags.get(i);
    			int temp1=0;
        		for(int j=0;j<cutPostion_use1.size();j++){
        			if(j==0){
        				temp1=0;
        			}else{
        				temp1=(int) (temp1+(cutPostion_use1.get(j)[0]-cutPostion_use1.get(j-1)[1]));
        			}
	...
	private void trimVideo(String workingPath, List<long[]> newSeeks){
		progressDialog = ProgressDialog.show(EditVedioActivity2.this,
				"裁剪中...", "请稍等...", true);
...
//		manager.trimVideo(EditVedioActivity2.this, file, storagePath, newSeeks, new CompletionListener() {
//			@Override
...
private void reRreshUI(){
	...
	private void saveFlag() {
        System.out.println("EditVedioActivity2   标记保存===="+mVedioPath);
