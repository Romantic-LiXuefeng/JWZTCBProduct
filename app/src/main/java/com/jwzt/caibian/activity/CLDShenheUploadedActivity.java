 ...
 *
 */
public class CLDShenheUploadedActivity extends BaseActivity implements OnClickListener {
	/**标题*/
			...
			case 1:
				if(mNewUploadinglistBean!=null){
					...
					if(IsNonEmptyUtils.isList(mList)){
                       ...
                       findViewById(R.id.rl_pre).setVisibility(View.GONE);
                    }
				}
				dismisLoadingDialog();
				break;
			case 11:
				Toast.makeText(CLDShenheUploadedActivity.this, "审核通过成功", 1).show();
				dismisLoadingDialog();
				CLDShenheUploadedActivity.this.finish();
				break;
			case 12:
                Toast.makeText(CLDShenheUploadedActivity.this, "审核驳回成功", 1).show();
				dismisLoadingDialog();
				CLDShenheUploadedActivity.this.finish();
				break;
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		application=(CbApplication) CLDShenheUploadedActivity.this.getApplication();
...
//        .displayer(new FadeInBitmapDisplayer(100))
	...
	private void initData(){
		if (CbApplication.getNetType() != -1) {
			showLoadingDialog(CLDShenheUploadedActivity.this, "", "");
	...
	private void findViews() {
		...
		LinearLayout	tijiaobaocun=(LinearLayout) findViewById(R.id.xiugaibaocun);
		tijiaobaocun.setOnClickListener(this);
		
		dianzanline.setOnClickListener(this);
		if(operateType.equals("4")){
			tijiaobaocun.setVisibility(View.GONE);
		}
		
		if(operateType.equals("2")&&Configs.firstChecks){//如果是一审稿件，并且有一审权限
			tijiaobaocun.setVisibility(View.VISIBLE);
		}else if(operateType.equals("3")&&Configs.secondChecks){
			tijiaobaocun.setVisibility(View.VISIBLE);
		}else if(operateType.equals("2")&&Configs.secondChecks){
			tijiaobaocun.setVisibility(View.VISIBLE);

		}else if(operateType.equals("1")&&Configs.firstChecks){
			tijiaobaocun.setVisibility(View.VISIBLE);
		}else{
			tijiaobaocun.setVisibility(View.GONE);
		}
...
//		wb_content.loadDataWithBaseURL(null, url, "text/html", "UTF-8", null);
	...
	private void addViews(){
		for (int i = 0; i < mList.size(); i++) {
			View view=View.inflate(CLDShenheUploadedActivity.this, R.layout.media_item_layout, null);
...
//					startActivity(intent);
					...
					if(fileRealPath.endsWith("mp4")||fileRealPath.endsWith("mp3")){
						Intent videointent=new Intent(CLDShenheUploadedActivity.this, ShowVideoActivity.class);
						videointent.putExtra("playpath", fileRealPath);
						startActivity(videointent);
					}else{
						Intent intent=new Intent(CLDShenheUploadedActivity.this, ShowImageActivity.class);
...
//						}
		...
		if(imgNum>0){
			tv_image_num.setVisibility(View.VISIBLE);
		}if(videoNum>0){
			tv_video_num.setVisibility(View.VISIBLE);
		}if(audioNum>0){
			tv_audio_num.setVisibility(View.VISIBLE);
		}
	...
	public void onClick(View v) {
		...
		case R.id.tv_edit://编辑按钮
			Intent intent=new Intent(CLDShenheUploadedActivity.this,NewScriptActivity.class);
		...
		case R.id.tv_all:
			if(mNewUploadinglistBean!=null){
				...
				if(IsNonEmptyUtils.isList(list)){
					Intent intent_all=new Intent(CLDShenheUploadedActivity.this,AllFootageActivity.class);
					intent_all.putExtra("alls",  (Serializable)list);
					startActivity(intent_all);
					overridePendingTransition(R.anim.push_left_in,R.anim.push_left_out);
				}else{
					UserToast.toSetToast(CLDShenheUploadedActivity.this, "没有全部内容");
				}
			}
	 ...
	 /**
	    ...
	    */
		private void showWindow() {
			...
			View view = LinearLayout.inflate(CLDShenheUploadedActivity.this,
			...
			imageButton_queding.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					if ("".equals(tv_contents.getText().toString())) {
						Toast.makeText(CLDShenheUploadedActivity.this, "请填写驳回理由", 1).show();
						return;
					}
					CommitData();
					tv_contents.setText("");
