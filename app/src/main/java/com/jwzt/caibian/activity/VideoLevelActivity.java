 ...
 *
 */
public class VideoLevelActivity extends BaseActivity implements OnClickListener {
	/**标题*/
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initList();
		findViews();
		framerateAdapter = new QualityAdapter(VideoLevelActivity.this,framerateList);
		resolutionAdapter=new QualityAdapter(VideoLevelActivity.this,resolutionList);
		bitrateAdapter=new QualityAdapter(VideoLevelActivity.this,bitrateList);
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int postion,
				...
				for (int i = 0; i < resolutionList.size(); i++) {
					if (i==postion) {
						...
						mSharePreferenceUtils.putInt(getApplicationContext(), GlobalContants.VIDEORESOLUTIONPOSION, postion);
					}else{
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				for (int i = 0; i < framerateList.size(); i++) {
					if (i==position) {
						...
						mSharePreferenceUtils.putInt(getApplicationContext(), GlobalContants.VIDEOlIVEVIDEOFBSPOSION, position);
					}else{
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				...
				for (int i = 0; i < bitrateList.size(); i++) {
					if (i==position) {
						...
						mSharePreferenceUtils.putInt(getApplicationContext(), GlobalContants.VIDEOVOICEBITRATEPOSION, position);
					}else{
	...
	private void findViews() {
		...
		int bitrate = mSharePreferenceUtils.getInt(UIUtils.getContext(), GlobalContants.VIDEOVOICEBITRATEPOSION, 3);
		bitrateList.get(bitrate).setSelected(true);
		gv_resolution=(GridView) findViewById(R.id.gv_resolution);
		int resolution = mSharePreferenceUtils.getInt(UIUtils.getContext(), GlobalContants.VIDEORESOLUTIONPOSION, 2);
		resolutionList.get(resolution).setSelected(true);
		gv_framerate=(GridView) findViewById(R.id.gv_framerate);
		int framerate = mSharePreferenceUtils.getInt(UIUtils.getContext(), GlobalContants.VIDEOlIVEVIDEOFBSPOSION, 0);
	...
	protected void onPause() {
		//这里存分辨率
		int TARGETRESOLUTIONPOSION = mSharePreferenceUtils.getInt(getApplicationContext(), GlobalContants.VIDEORESOLUTIONPOSION, 2);
			mSharePreferenceUtils.putInt(getApplicationContext(), GlobalContants.VIDEORESOLUTION_W,
					newResolutionList.get(TARGETRESOLUTIONPOSION).videoResolution_w);
			mSharePreferenceUtils.putInt(getApplicationContext(), GlobalContants.VIDEORESOLUTION_H,
					newResolutionList.get(TARGETRESOLUTIONPOSION).videoResolution_h);
		//这里存帧率
		int lIVEVIDEOFBSPOSION = mSharePreferenceUtils.getInt(UIUtils.getContext(), GlobalContants.VIDEOlIVEVIDEOFBSPOSION, 0);
		mSharePreferenceUtils.putInt(UIUtils.getContext(), GlobalContants.VIDEOlIVEVIDEOFBS, newFramerateList.get(lIVEVIDEOFBSPOSION).videoFramera);
			
		//这里存音频的码率
		int VOICEBITRATEPOSION = mSharePreferenceUtils.getInt(UIUtils.getContext(), GlobalContants.VIDEOVOICEBITRATEPOSION, 3);
		mSharePreferenceUtils.putInt(UIUtils.getContext(), GlobalContants.VIDEOVOICEBITRATE,newBitrateList.get(VOICEBITRATEPOSION).voiceBitrate);
		super.onPause();
	}
