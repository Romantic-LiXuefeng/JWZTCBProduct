...
import com.jwzt.caibian.xiangce.FloderAdapter;
import com.jwzt.caibian.xiangce.ItemImage;
import com.jwzt.caibian.xiangce.OtherUtils;
import com.jwzt.caibian.xiangce.Photo;
import com.jwzt.caibian.xiangce.PhotoAdapter;
import com.jwzt.caibian.xiangce.PhotoFloder;
import com.jwzt.caibian.xiangce.PhotoUtils;
...
public class PhotoPickerActivity extends BaseActivity implements PhotoAdapter.PhotoClickCallBack {
    ...
    public final static String ALL_TYPE="alltype";
    public final static String ALL_PHOTO = "所有图片";
    public final static String ALL_AUDIO = "所有音频";
    public final static String ALL_VIDEO = "所有视频";
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		mShowType=getIntent().getStringExtra(ALL_TYPE);
        initIntentParams();
        initView();
    ...
    private void getPhotosSuccess() {
        ...
        if(IsNonEmptyUtils.isList(mFloderMap.get(mShowType).getPhotoList())){
        	mPhotoLists.addAll(mFloderMap.get(mShowType).getPhotoList());
...
//            mCommitBtn.setText(OtherUtils.formatResourceString(getApplicationContext(),R.string.commit_num, list.size(), mMaxNum));
            mCommitBtn.setText(OtherUtils.formatResourceString(getApplicationContext(),R.string.commit_selectnum, list.size()));
        } else {
    ...
    private void returnData() {
        ...
        for(int i=0;i<mSelectList.size();i++){
        	...
        	item.setFilepath("file:///"+mSelectList.get(i));
        	item.setResult(false);
        	Bimp.tempSelectBitmap.add(0,item);
        }
...
//            mFloderMap = PhotoUtils.getPhotos(PhotoPickerActivity.this.getApplicationContext());
        	if(mShowType.equals(ALL_PHOTO)){
        		mFloderMap = PhotoUtils.getPhotos(PhotoPickerActivity.this.getApplicationContext());
        	}else if(mShowType.equals(ALL_VIDEO)){
        		mFloderMap = PhotoUtils.getVideoFile1(PhotoPickerActivity.this.getApplicationContext());
        	}else if(mShowType.equals(ALL_AUDIO)){
        		mFloderMap = PhotoUtils.getAudio(PhotoPickerActivity.this.getApplicationContext());
        	}else{
        		mFloderMap = PhotoUtils.getPhotos(PhotoPickerActivity.this.getApplicationContext());
        	}
            return null;
