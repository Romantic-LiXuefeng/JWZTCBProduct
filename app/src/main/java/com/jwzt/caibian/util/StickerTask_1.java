...
import com.jwzt.caibian.activity.StirckerActivity;
 ...
 * 贴图合成任务 抽象类
 */
public abstract class StickerTask_1 extends AsyncTask<Bitmap, Void, Bitmap> {
    private Dialog dialog;

    private StirckerActivity mContext;

    public StickerTask_1(StirckerActivity activity) {
    ...
    protected Bitmap doInBackground(Bitmap... params) {
        ...
        BitmapUtils.saveBitmap(resultBit, mContext.filePath);//保存操作的原图
        BitmapUtils.saveBitmap(resultBit, mContext.new_filePath_sub);//保存操作后的缩略图
        return resultBit;
    }
