...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(CbApplication.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(CbApplication.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private boolean cacheBitmap2Disk(Bitmap bitmap, OutputStream outputStream) {
    	...
    	System.out.println("bitmap"+outputStream);
    	if(bitmap!=null){
