...
//		sGlideRequests = GlideApp.with(mContext);
	...
	@Override
	public void setListenerForImageView(ImageView imageView,int size,int color) {
		imageView.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				System.out.println("========================ARE_StyleImage");
			//	openImageChooser();
				//openImageChooser();
	...
	public void insertImage(final Object src, final ImageType type) {
		...
		System.out.println("URI======================="+src);

        if (type == ImageType.URI) {

			//Glide.with(mContext).load((Uri) src).asBitmap().centerCrop().transform(new CenterCrop(mContext),new GlideRoundImage(mContext));
            Glide.with(mContext).load((Uri) src).asBitmap().centerCrop().into(myTarget);
        } else if (type == ImageType.URL) {
//			Glide.with(mContext).load((String) src).asBitmap().centerCrop().transform(new CenterCrop(mContext),new GlideRoundImage(mContext));
